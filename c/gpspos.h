...
/*
	...
	can compare with gps module provide track info
*/
...
*/
double gpsbearing(double pos1[2], double pos2[2]) {
...
/*
	...
	returns metres
*/
double gpsdistance(double pos1[2], double pos2[2]){
...
double crosstrack(double pos1[2], double pos2[2], double prespos[2]){
	...
	origbear = gpsbearing(pos1, prespos) - gpsbearing(pos1, pos2);
	if(origbear <= 0){
		origbear = origbear + 360;
	} else if (origbear > 360){
		origbear = origbear - 360;
	}
	destbear = gpsbearing(pos2, pos1) - gpsbearing(pos2, prespos);
	if(destbear <= 0) {
		destbear = destbear + 360;
	} else if (destbear > 360){
		destbear = destbear - 360;
	}
	origdist = gpsdistance(pos1, prespos);
	destdist = gpsdistance(prespos, pos2);
...
double tofrom(double dest[2], double prespos[2], double lastpos[2]){
	...
	track = gpsbearing(lastpos, prespos);
	bearing = gpsbearing(prespos, dest);
